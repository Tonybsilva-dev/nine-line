...
import { ApproveAppointmentUseCase } from '../../application/use-cases/approve-appointment/approve-appointment.use-case';
...
export async function approveAppointmentController(
  req: Request,
  res: Response,
) {
  const { id } = req.params;
  ...
  const useCase = new ApproveAppointmentUseCase(
