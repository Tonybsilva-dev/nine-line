...
describe('FindAppointmentsByUserIdUseCase', () => {
  ...
  beforeEach(() => {
    ...
    findAppointmentsByUserIdUseCase = new FindAppointmentsByUserIdUseCase(
  ...
  it('should return paginated appointments with total', async () => {
    ...
    await appointmentRepository.create(
      makeAppointment({ userId: 'other-user' }),
    );
  ...
  it('should return empty list when no appointments exist for user', async () => {
    const result = await findAppointmentsByUserIdUseCase.execute('user-123', {
      page: 1,
      perPage: 10,
    });
