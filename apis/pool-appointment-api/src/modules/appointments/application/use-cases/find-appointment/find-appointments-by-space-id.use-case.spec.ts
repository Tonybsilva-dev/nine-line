...
describe('FindAppointmentsBySpaceIdUseCase', () => {
  ...
  beforeEach(() => {
    ...
    findAppointmentsBySpaceIdUseCase = new FindAppointmentsBySpaceIdUseCase(
  ...
  it('should return paginated appointments with total', async () => {
    ...
    await appointmentRepository.create(
      makeAppointment({ spaceId: 'other-space' }),
    );
  ...
  it('should return empty list when no appointments exist for space', async () => {
    const result = await findAppointmentsBySpaceIdUseCase.execute('space-123', {
      page: 1,
      perPage: 10,
    });
  ...
  it('should return correct page of appointments', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      await appointmentRepository.create(
        makeAppointment({ spaceId, startTime, endTime }),
      );
    }
