...
export class InMemoryAppointmentRepository implements AppointmentRepository {
  ...
  async findByUserId(
    ...
    const appointments = userAppointments.slice(
      skip,
      skip + (params.perPage ?? 10),
    );
  ...
  async findBySpaceId(
    ...
    const spaceAppointments = this.appointments.filter(
      (a) => a.spaceId === spaceId,
    );
    const skip = ((params.page ?? 1) - 1) * (params.perPage ?? 10);
    const appointments = spaceAppointments.slice(
      skip,
      skip + (params.perPage ?? 10),
    );
  ...
  async update(appointment: Appointment): Promise<void> {
    const index = this.appointments.findIndex(
      (a) => a.id.toString() === appointment.id.toString(),
    );
    if (index >= 0) {
